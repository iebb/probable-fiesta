...
def parse_url_inside_css(css_file, css_url, filename):
    url_patterns = re.findall(r'url\(["\']?([^)]*?)["\']?\)', css_file)
    for url in url_patterns:
        if url.startswith("data:"):
            continue
        elif url.startswith("https:") or url.startswith("http:"):
            css_file = css_file.replace(url, crawl_single_file(url))
        elif url.startswith("/"):
            css_abs_url = urllib.parse.urljoin(css_url, url)
            css_file = css_file.replace(url, crawl_single_file(css_abs_url))
        else: # relative path
            if url:
                css_abs_url = urllib.parse.urljoin(css_url, url)
                target_url = urllib.parse.urljoin(filename, url)
                crawl_single_file(css_abs_url, target_url)
    return css_file


def crawl_single_file(url, filepath=None):
    if "://" not in url:
        return url
    ...
    # aaa.css?q=1a2b3c => aaa.q_1a2b3c.css to preserve type
    ...
    if filepath.endswith(".css"):
        ...
        content = parse_url_inside_css(crawled.text, url, filepath)
...
def crawl_page(base_link):
    ...
    save_location = get_main_save_link(base_link)
    ...
    for tag in soup.find_all('link', rel='stylesheet'):  # link-stylesheet tag
        if tag.has_attr('href'):
            ...
            tag['href'] = crawl_single_file(_href)
    ...
    for tag in soup.find_all('script', src=True):  # script tag, src
        ...
        tag['src'] = crawl_single_file(_href)

    for tag in soup.find_all('style'):  # embedded styles. parse them for css urls
        parse_url_inside_css(str(tag), base_link, save_location)
        ...
        if tag.has_attr('srcset'):
            ...
            for srcset_item in tag['srcset'].split(","):
                ...
                if len(stripped) == 2:
                    ...
                    new_url = crawl_single_file(urllib.parse.urljoin(base_link, srcset_items[0]))
    ...
    if os.path.isfile(save_location):
        with open(save_location, "r", encoding="u8") as saved_file:
            ...
            if soup.children:
                ...
                if type(cmt) == Comment and cmt.startswith(metadata_identifier):
